...
def extract_values(load_list, attr_name):
    return [
        getattr(load, attr_name)
        for load in load_list
        if getattr(load, attr_name) is not None
    ]
...
def calculate_average_loads(
    ...
    for instance_id, load_level_info in nf_load_level_infos_by_instance.items():
        ...
        averages = {
            field: int(average(extract_values(load_level_info, field)))
            for field in FIELDS
        }

        # 집계된 값들 설정
        load.nf_cpu_usage = averages["nf_cpu_usage"]
        load.nf_memory_usage = averages["nf_memory_usage"]
        load.nf_storage_usage = averages["nf_storage_usage"]
        load.nf_load_level_average = averages["nf_load_level_average"]
        load.nf_load_levelpeak = averages["nf_load_levelpeak"]
        load.nf_load_avg_in_aoi = averages["nf_load_avg_in_aoi"]
