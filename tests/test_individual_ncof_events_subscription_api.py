...
def test_delete_ncof_events_subscription_missing_id(client: TestClient):
    """Test case for delete_ncof_events_subscription with missing subscriptionId"""
    ...
    response = client.request(
        "DELETE",
        "/subscriptions/x",  # subscriptionId 없이 요청
        headers=headers,
    )

    assert response.status_code == 404  # FastAPI는 경로 파라미터가 없으면 404 반환
